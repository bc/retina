...
test_that("we can compute different versions of the retinaplot smoothing with lamdba and polynomial values", 
    {
        ...
        poly <- c(2, 3, 4, 5)
        lambda <- c(0.01, 0.001, 1e-04, 1e-05)
        # Generate error vectors for each combination Save each plot to a PDF in the
        # working directory
        
        retina_A <- main_3hgbqg("test_retinas/3hgbqg/diagram_retina/")
        ERR_dat <- polynomial_vs_lambda(retina_A, spatial_res = RES_variable, polynomial_m_vec = poly, 
            lambda_vec = lambda)
        ...
        the_types <- c("X_median", "X_min", "X_max", "X_mean", "X_sd", "X_range_len")
        
        for (e in the_types) {
            err_obj <- eval(parse(text = e))
            boxplot(err_obj, horizontal = TRUE, las = 1, col = "lightgray", cex = 0.2, 
                pch = 20, sub = paste("n = ", sampling_location_number), cex.axis = 0.4, 
                main = paste0("Ordered by ", e))
        }
        
        boxplot(X_sd, horizontal = TRUE, las = 1, col = "lightgray", asp = 1, cex = 0.2, 
            pch = 20, sub = paste("n = ", sampling_location_number), cex.axis = 0.4, 
            main = paste0("Ordered by sd"))
